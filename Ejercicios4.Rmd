---
title: "Ejercicio 4"
author: "Ignacio Raul Morales Berber"
...
{\Huge Universidad del Estado de Mexico}\\[0.5cm]
{\Large En Linea}\\[1cm]

{\LARGE \textbf{Ejercicio 4}}\\[1cm]

{\Large Materia: Estadistica Descriptiva e Inferencia}\\
{\Large Profesora: Shurabe Guido Aguilar}\\[1cm]

\textbf{Alumnos:} \\
Alexis Xiomara Villegas Cortes \\ 
Denisse Aurora Alvarado Reyna \\ 
Ignacio Raul Morales Berber \\[3cm]
...
sprintf("Probabilidad por metodo manual %s",resultado)
...
sprintf("Numero de pacientes esperados que tengan efectos secundarios: %s", media)
...
sprintf("Con desviacion estandar: %s", desviacion)
```
...
sprintf("Probablidad de que se reciba mas de una llamada en los proximos 15 min: %s", probability)

k<-1
sprintf("Calculo usando R: %s", ppois(k,lambda = lambda_value,lower.tail = FALSE))
```
...
sprintf("probabilidad de que se encuentren exactamente 20 bacterias en una muestra de 50 ml: %s",probability)
...
probability <- 0.1056
sprintf("La probabilidad de una accion tenga un costo menor que $20 usando Z: %s es %s",Z,  probability)
